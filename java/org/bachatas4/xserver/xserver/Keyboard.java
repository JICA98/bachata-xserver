...
public class Keyboard {
    ...
    public boolean onKeyEvent(KeyEvent event) {
        if (event.getDevice() != null && (event.getDevice().getSources() & android.view.InputDevice.SOURCE_GAMEPAD) != 0) return false;
        ...
        if (action == KeyEvent.ACTION_DOWN || action == KeyEvent.ACTION_UP) {
            ...
            if (chars != null && chars.length() == 1) {
                ...
                new Handler(Looper.getMainLooper()).postDelayed(() -> xServer.injectKeyRelease(xKeycode), 30);
            }
        }
        return true;
    }
